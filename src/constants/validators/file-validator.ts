...
type FileType =IMulterFile |IMulterFile[] | Record<string,IMulterFile[]>;
type Result = { errorFileName?: string; isValid: boolean };

export const runFileValidation = async (args: {
  multiple: boolean;
  file: FileType;
  validator: (file:IMulterFile) => Promise<boolean> | boolean;
}): Promise<Result> => {
  if (args.multiple) {
...
export class FileSizeValidator extends FileValidator {
  ...
  async isValid(
    file?:IMulterFile |IMulterFile[] | Record<string,IMulterFile[]>,
  ): Promise<boolean> {
...
export class FileTypeValidator extends FileValidator {
  ...
  isMimeTypeValid(file:IMulterFile) {
